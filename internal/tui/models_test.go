...
func TestApp_ModelsCommand_AsyncFetchAndCache(t *testing.T) {
	...
	assert.True(t, a.modelDialog.open, "/models should open the model picker dialog")
	view := a.View()
	...
	assert.True(t, a.modelDialog.open, "cache hit should reopen model picker")
	assert.Equal(t, "claude", a.modelDialog.query)
	view = a.View()
